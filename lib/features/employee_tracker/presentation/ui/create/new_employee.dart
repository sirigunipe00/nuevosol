...
import 'package:nuevosol/features/employee_tracker/presentation/ui/create/employee_form_widget.dart';
...
class _NewEmployeeState extends State<NewEmployee> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar:
          isNew
              ? const SimpleAppBar(title: 'New Gate Pass')
              : TitleStatusAppBar(
                    title: 'Gate Pass',
                    docNo: name.valueOrEmpty,
                    status: StringUtils.docStatusEmployee(status.toString()),
      ...
      body: BlocListener<CreateEmployeeCubit, CreateEmployeeState>(
        listener: (_, state) async {
          if (state.isSuccess && state.successMsg.isNotNull) {
            if (state.form.workflowState == 'Rejected') {
              AppDialog.showErrorDialog(
                ...
                context.cubit<EmployeeEntriesCubit>().fetchInitial(
                  Triple(
                    ...
                    filter.query,filter.department
                  ),
                );
